...
public class TestGame extends Game {
	...
	/*
	 * Material material2 = new Material();// new Texture("test.png"), new
	 * // Vector3f(1,1,1), 1, 8); material2.addTexture("diffuse", new
	 * Texture("test.png")); material2.addFloat("specularIntensity", 1);
	...
	// GameObject testMesh1 = new GameObject().addComponent(new
	// MeshRenderer(mesh2, material));
	// GameObject testMesh2 = new GameObject().addComponent(new
	// MeshRenderer(mesh2, material));
	GameObject watchingMonkey = new GameObject().addComponent(new TrackMainCameraComponent()).addComponent(new MeshRenderer(tempMesh, material));

	// testMesh1.getTransform().getPos().set(0, 2, 0);
	// testMesh1.getTransform().setRot(new Quaternion(new Vector3f(0, 1, 0),
	// 0.4f));
	...
	GameObject camera = new GameObject().addComponent(new FreeLook(0.5f)).addComponent(new FreeMove(10.0f)).addComponent(new Camera((float) Math.toRadians(70.0f), (float) Window.getWidth() / (float) Window.getHeight(), 0.01f, 1000.0f));

	// addObject(testMesh1);
	addObject(watchingMonkey);

	watchingMonkey.getTransform().getPos().set(5, 5, 5);
	watchingMonkey.getTransform().setRot(new Quaternion(new Vector3f(0, 1, 0), (float) Math.toRadians(-70.0f)));

	// addObject(new GameObject().addComponent(new MeshRenderer(new
	// Mesh("monkey3.obj"), material2)));

	directionalLight.getTransform().setRot(new Quaternion(new Vector3f(1, 0, 0), (float) Math.toRadians(-45)));
	addObject(camera);

	camera.getTransform().getPos().set(watchingMonkey.getTransform().getPos());
	camera.getTransform().getPos().setX(camera.getTransform().getPos().getX() + 5);
	camera.getTransform().getRot().set(watchingMonkey.getTransform().getRot());
    }
}
